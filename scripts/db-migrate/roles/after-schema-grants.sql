...
-- Connect as the short-lived migrator; its only elevated action is this
-- explicit switch to the non-login schema owner.
SET ROLE saveswitch_owner;

-- Run as saveswitch_owner only after the ledger has reached 0004 and before
-- the canonical data-only restore. This intentionally names the three load
-- tables: loader never receives rights to asset_deletion_queue.
...
-- The application can perform its normal data lifecycle but has no CREATE,
-- ALTER, DROP, TRUNCATE, database ownership, or migration-ledger access.
...
-- Future table access must be deliberately reviewed rather than inherited by
-- application or loader roles.
